...
void Session::waitForRequest()
{
	...
	[this, self](boost::system::error_code ec, std::size_t) {
			if (!ec) {
				...
				waitForRequest(); // Continue listening for more messages
			}
			else {
				...
				server_.broadcast(id_, "has beed disconnected");
				cleanUp();
			}
		});
}
...
void Server::broadcast(int senderId, const std::string& message)
{
	...
	for (const auto& session : sessions_) {
		...
		session->sendMessage("[Client " + std::to_string(senderId) + "] " + message);
	}
}
